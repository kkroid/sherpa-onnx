...
int main() {
    ...
    SherpaOnnxExpression *expression = SherpaOnnxPCM2Expression(session, samples.data(), n, sample_rate);
    auto end_time = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
    SHERPA_ONNX_LOGE("Inference duration: %d ms", duration.count());
    if (expression == nullptr) {
        SHERPA_ONNX_LOGE("Failed to infer expression");
        return -1;
    }
    ...
    output_file.close();
    SherpaOnnxDestroyExpression(expression);
    SherpaOnnxDestroyOrtSession(session);
